...
for zz = 1:numel(id)
    for ii = 1:numel(simtype)
        ...
        files.D       = fullfile(files.root,'sims',[sprintf('%03d',id(zz)) '_' simtype{ii} '_sim_' num2str(snr) 'dB_001.mat']);
        %     files.simroot = 'D:\Documents\ebb_corr\paper_sims';
        %     files.D       = fullfile(files.simroot,[simtype{ii} '_sim_' num2str(snr) 'dB_002.mat']);
        files.results = fullfile(files.root,'proc',[sprintf('%03d',id(zz)) '_' simtype{ii} '_' num2str(snr) 'dB']);
...
for zz = 1:numel(id)
    ...
    for ii = 1:numel(simtype)
        for jj = 1:numel(inversions)
            ...
            files.results = fullfile(files.root,'proc',[sprintf('%03d',id(zz)) '_' simtype{ii} '_' num2str(snr) 'dB'],inversions{jj});
            files.BF      = fullfile(files.results,'BF.mat');
            ...
            count = count + 1;
            matlabbatch{count}.spm.tools.beamforming.copy.BF = {fullfile(files.results,'..','BF.mat')};
            matlabbatch{count}.spm.tools.beamforming.copy.dir = {files.results};
            matlabbatch{count}.spm.tools.beamforming.copy.steps = 'all';
            
            % Empirical Bayesian source reconstuction
            count = count + 1;
            matlabbatch{count}.spm.tools.beamforming.inverse.BF = {files.BF};
            switch inversions{jj}
                case 'EBB'
                    matlabbatch{count}.spm.tools.beamforming.inverse.plugin.ebb.corr = false;
                case 'EBBcorr'
                    matlabbatch{count}.spm.tools.beamforming.inverse.plugin.ebb.corr = true;
            end
            matlabbatch{count}.spm.tools.beamforming.inverse.plugin.ebb.keeplf = false;
